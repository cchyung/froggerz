...
public class Websocket {
    ...
    private native void _open(Websocket ws, String s, String url) /*-{
        ...
        $wnd[s].onopen = function() { ws.@froggerz.websockets.Websocket::onOpen()(); };
        $wnd[s].onclose = function(evt) { ws.@froggerz.websockets.Websocket::onClose(SLjava/lang/String;Z)(evt.code, evt.reason, evt.wasClean); };
        $wnd[s].onerror = function() { ws.@froggerz.websockets.Websocket::onError()(); };
        $wnd[s].onmessage = function(msg) { alert(msg.data); ws.@froggerz.websockets.Websocket::onMessage(Ljava/lang/String;)(msg.data); alert("After onMessage call");}
    }-*/;
    ...
    protected void onMessage(String msg) {
        for (WebsocketListener listener : listeners) {
        	debug("Before onMessage listener"); // DEBUG
            listener.onMessage(msg);
            debug("After onMessage listener"); // DEBUG
